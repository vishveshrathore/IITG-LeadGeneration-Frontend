...
const RejectedLeads = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch(`${BASE_URL}/api/lg/rejected?page=${page}&limit=${limit}`, {
        ...
        console.error("Failed to load rejected leads", e);
        toast.error(e?.message || 'Failed to load rejected leads');
      } finally {
        setLoading(false);
      }
    };
  ...
  const saveEdit = async () => {
    try {
      ...
      if (editType === 'RawLead') {
        // RawLead requires full payload with required fields
        const payload = {
          name: editName,
        ...
        setRows(prev => prev.map(x => {
          ...
          // Preserve populated names if server didn't include them
        ...
        // HR update via broader endpoint available in LG routes
        const payload = {
          name: editName,
          designation: editDesignation,
          mobile: mobileArr,
        ...
        setRows(prev => prev.map(x => {
          if (x._id !== editLead._id) return x;
          const merged = { ...x, ...payload, ...serverDoc };
          // Keep previously populated relations (editHr doesn't populate)
    ...
    <div className="min-h-screen w-full bg-slate-50 text-slate-900">
      ...
      <div className="pt-20 px-6 w-full mb-10">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Rejected Leads</h1>
          <div className="flex items-center gap-2">
            <button className="px-3 py-2 rounded border bg-white hover:bg-slate-50" onClick={()=>navigate('/lgDashboard')}>Dashboard</button>
            <button className="px-3 py-2 rounded border bg-white hover:bg-slate-50" onClick={()=>navigate('/lg/viewtodaysleads')}>Todays Leads</button>
          </div>
        </div>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td className="px-4 py-6" colSpan={7}>No rejected leads</td></tr>
                ) : (
                  rows.map((l) => (
                    <React.Fragment key={l._id}>
                      <tr className="border-t">
                        ...
                        <td className="px-4 py-2">
                          <span className={`px-2 py-0.5 rounded text-xs ${l.type === 'RawLead' ? 'bg-orange-100 text-orange-700' : 'bg-indigo-100 text-indigo-700'}`}>{l.type}</span>
                        </td>
          ...
          <motion.div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center"
            ...
            <motion.div className="bg-white rounded-xl shadow-lg w-full max-w-2xl p-6"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {/* Read-only company/industry names for RawLead context */}
                {editType === 'RawLead' && (
                  <>
                    <div>
