...
const navItems = [
  { name: 'Dashboard', path: '/recruitment-qc/dashboard' },
  { name: 'Position MIS', path: '/admin/recruitment/position-mis' },
  { name: 'Local Hiring', path: '/recruitment-qc/local-hiring' },
];
...
const RecruitmentQCLocalHiring = () => {
  ...
  const openEmailModalForCurrentLead = () => {
    if (!lead) return;
    const candidateEmail = (emailInput || lead.email || lead?.profile?.email || '').trim();
    if (!candidateEmail) {
    ...
    setEmailModalTo(candidateEmail.toLowerCase());
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (emailOpportunity) {
        const jdKeys = [];
        if (emailOpportunity.includes('CRE')) jdKeys.push('cre');
        if (emailOpportunity.includes('CRM')) jdKeys.push('crm');
        if (emailOpportunity.includes('Mgr HRBP')) jdKeys.push('mgr_hrbp');
        if (emailOpportunity.includes('Team Lead CRM')) jdKeys.push('team_lead_crm');
        if (emailOpportunity.includes('Training & Productivity')) jdKeys.push('training_productivity');
        const primaryJdKey = jdKeys[0] || '';
        if (primaryJdKey) formData.append('jdKey', primaryJdKey);
        if (jdKeys.length) formData.append('jdKeys', JSON.stringify(jdKeys));
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#fef7ff] via-[#f4edff] to-[#f8fbff]">
      <AnimatedHRNavbar title="Recruitment / QC Manager" navItems={navItems} />
              ...
              <div className="flex gap-2 bg-slate-50 rounded-2xl p-1">
                {['next', 'worksheet'].map((key) => (
                  <button
                    ...
                    onClick={() =>
                      key === 'worksheet'
                        ? navigate('/recruitment-qc/local-hiring/worksheet')
                        : setTab(key)
                    }
                          ...
                          {getProfileFields(lead)
                            ...
                            .map((f) => {
                              ...
                              return (
                                <div
                                  key={f.label}
                                  className="flex items-start gap-3 rounded-2xl border border-slate-100 bg-gradient-to-br from-slate-50/90 to-white px-3 py-2 shadow-sm"
                                >
...
export default RecruitmentQCLocalHiring;
